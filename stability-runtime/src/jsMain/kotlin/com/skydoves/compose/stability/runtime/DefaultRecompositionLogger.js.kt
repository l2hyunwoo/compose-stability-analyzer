...
/**
 * JavaScript implementation of DefaultRecompositionLogger that uses console.log.
 *
 * Example output:
 * ```
...
public actual class DefaultRecompositionLogger : RecompositionLogger {
  ...
  actual override fun log(event: RecompositionEvent) {
    ...
    event.parameterChanges.forEachIndexed { index, change ->
      ...
      console.log("$prefix ${change.name}: ${change.type} $status")
    }

    // Log unstable parameters summary
    if (event.unstableParameters.isNotEmpty()) {
      console.log("  └─ Unstable parameters: ${event.unstableParameters}")
    }
  }

  /**
   * Safely converts a value to string, handling function types and errors.
  ...
  private fun safeToString(value: Any?): String {
    ...
    return try {
      ...
      // Fallback for any toString() failures
